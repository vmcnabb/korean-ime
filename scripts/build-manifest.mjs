...
// The OSK header mode indicator inlines these icons as data URLs; rendered
// oversized (it's displayed at ~16px) so it stays crisp on high-DPI / scaled
// displays, the way the toolbar icon does via its multi-size set.
const modeIconsModulePath = resolve(root, "src/content-script/on-screen-keyboard/mode-icons.ts");
const modeIconRenderSize = 48;
...
function generateModeIcons() {
    const dataUrl = (suffix) =>
        `data:image/png;base64,${renderIconPng(actionIconSources[suffix], modeIconRenderSize).toString("base64")}`;
    ...
    const moduleSource =
        ...
        `export const modeIconHangul = "${dataUrl("h")}";\n` +
        `export const modeIconEnglish = "${dataUrl("a")}";\n`;
