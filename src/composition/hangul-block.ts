...
export class HangulBlock {
    ...
    toChar(): string {
        ...
        const medialIndex = this.medial.length == 1
            ? medials.indexOf(this.medial)
            : medials.indexOf(compoundVowels[this.medial]!);

        const finalIndex = (this.final.length == 1
    ...
    static fromChar(character: string, separateMedialDigraph = true, separateFinalDigraph = true) {
        ...
        return new HangulBlock(
            ...
            separateMedialDigraph && compoundVowels[medials[medialIndex]] || medials[medialIndex],
            separateFinalDigraph && consonantDigraphs[finals[finalIndex]] || finals[finalIndex]
        );
    }
}
