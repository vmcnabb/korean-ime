import {
    ...
    createSection,
    createSystemSetting,
} from "./option-definitions";
import { createPersistenceOption } from "./option-enums";

export type SettingsStore = SimplifySettings<
    ReturnType<typeof createDefaultSettings>["options"]
>;

export function createDefaultSettings() {
    return createSection("Korean IME Options", {
        onScreenKeyboard: createSection("On-screen Keyboard", {
            persist: createPersistenceOption(
                "State of the on-screen keyboard when the browser starts."
            ),
        ...
        hanYongToggle: createSection("Han/Yong Toggle", {
            persist: createPersistenceOption(
                "State of the Han/Yong toggle when the browser starts."
            ),
            lastState: createSystemSetting(false),
            shareStateAcrossTabs: createCheckBoxOption(
                "Share state across tabs",
